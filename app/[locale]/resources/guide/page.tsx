...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildResourceMetadata(locale, 'resourcesGuide', '/resources/guide');
}

export default async function GuidePage({ params }: PageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'resourcesGuide' });
  ...
  return (
    <ResourceChrome
      ...
      maxWidth={900}
    >
      <CountryBlock flag="🇸🇬" title={t('sg.title')}>
      ...
      <CountryBlock flag="🇯🇵" title={t('jp.title')}>
